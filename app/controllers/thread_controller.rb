class ThreadController < ApplicationController
  ...
  def new
    ...
    if @newthread.save
      image = params[:image]
      @newthread.image.attach(params[:image])

      #   ActiveStorageのためにコメントした
      # if image
  ...
  def image_changed
    @thread = AddThread.find_by(id: params[:thread_id])
    image = params[:image]
    if @thread.image.attached?
      @thread.image.purge #既存画像を削除する。
    end
    @thread.image.attach(params[:image])
    
    
    # ActiveStorageのためにコメントアウト化
